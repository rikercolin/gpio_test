...
static void gpio_list(const char* dev_name) {
    ...
    for (int i = 0; i < info.lines; i++) {
        ...
        if (ret == -1) {
            printf("Unable to get line info from offset %d: %s", i,
                strerror(errno));
        }
        else {
            printf(
...
static void help(const char* app) {
    ...
    fprintf(stderr,
        ...
        "\t -w <offset>: Write GPIO value at offset (OUTPUT mode). Option -v "
        "should be set\n"

        "\t -v <0|1>: value that should be written to the GPIO, used only "
        "with option -w\n"
